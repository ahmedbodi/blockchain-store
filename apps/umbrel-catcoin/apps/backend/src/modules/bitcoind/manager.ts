...
function onLine(src: Readable, callback: (line: string) => void) {
	...
	src.on('error', (err) => console.error('[catcoind-manager] stream error:', err))

	const rl = readline.createInterface({input: src})

	// Prevent an unhandled error from the readline from crashing the process
	rl.on('error', (err) => console.error('[catcoind-manager] readline error:', err))
	...
	rl.on('line', (raw) => {
		...
		try {
			...
			console.error('[catcoind-manager] onLine callback error:', err)
		}
	})
}
...
export class BitcoindManager {
	...
	constructor({binary = BITCOIND_BIN, datadir = BITCOIN_DIR, extraArgs = []}: BitcoindManagerOptions = {}) {
		...
		const envArgs = (process.env['CATCOIND_EXTRA_ARGS'] ?? '')
			.trim()
		...
		try {
			...
			console.error('[catcoind-manager] failed to get static version:', error)
		}
	}
	...
	start() {
		...
		this.lastError = null
		console.log('[catcoind-manager] spawned PID', this.child.pid)
		...
		this.child.on('exit', (code, sig) => {
			console.error(`[catcoind] exited (code=${code}, sig=${sig})`)
			...
			this.exitInfo = {
				...
				message: `Catcoin Core stopped (code ${code ?? 'null'})`,
			}
		...
		this.child.on('error', (err) => {
			console.error('[catcoind-manager] failed to spawn:', err)
			this.lastError = err
		})
	}
