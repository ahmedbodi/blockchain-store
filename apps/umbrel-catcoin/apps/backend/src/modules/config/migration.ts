...
const LEGACY_CONFIG_PATH = path.join(APP_STATE_DIR, 'catcoin-config.json')
...
export async function migrateLegacyConfig(): Promise<SettingsSchema | undefined> {
	// If the old catcoin-config.json doesn't exist, we don't need to migrate
	...
	const legacyConfig = await fse.readJson(LEGACY_CONFIG_PATH).catch((err) => {
		...
		const msg =
			err instanceof SyntaxError
				? '[migration] Invalid JSON in legacy catcoin-config.json'
				: '[migration] Unable to read legacy catcoin-config.json'
...
// Previous app's catcoin-config.json for reference:
